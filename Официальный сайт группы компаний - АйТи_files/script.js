...
function mainMenuHover() {
	...
	if($(window).width() > 769){
		$('body.main-page .g-menu li').hover(function () {
			otbThisObj = $(this);
			//console.log("hover");
			ITBintervalID = setInterval(function(){
				//console.log($(this));
				//console.log("showStart");
				otbThisObj.find("a").eq(0).css("background-position","300px 37px")
				otbThisObj.find('.sub-g-menu').slideDown(100);
				//console.log("showStop");
				clearInterval(ITBintervalID);
			},400)

		}, function () {
			//console.log("blur");
			//console.log("hideStart");
			otbThisObj.find("a").eq(0).css("background-position","177px 37px")
...
return false;
	...
	$('a[href="/en/callback/"],  input[data-form], a.vacancy-form').click(function(e) {
		var src, 
			clearModal = false;
...
$(document).ready(function() {
	if($(".itb_media_ajax").length >0){
		$(".itb_media_ajax").fancybox({
		  type: 'ajax',
		//  maxWidth : 800,
		//  maxHeight : 600,
		  fitToView : false,
	...
	if($(".itb_right_menu").length > 0){
		$('.itb_right_menu ul.itb_js_main_ul_right_menu > li').click(function(e){
			if($(this).hasClass("active")){
				$(this).removeClass("active")
			}else{
				$('.itb_right_menu ul.itb_js_main_ul_right_menu > li.active').removeClass("active");
				$(this).addClass("active");
			}
			
			
		});
		$('.itb_right_menu ul.itb_js_main_ul_right_menu > li > a').click(function(e){
			e.preventDefault();
		})
		$(".itb_second_lvl").click(function(e){
			e.stopPropagation();
		})
	}
	if($(".g-menu > ul > li > a").length > 0){
		$(".g-menu > ul > li > a").click(function(e){
			e.preventDefault();
		})
	}


	//открытие подпунктов главного меню
	var mainNavUl = $('div.top-menu > ul, .g-menu > ul'),
		mainNavLi = $('div.top-menu > ul > li, .g-menu > ul > li'),
		mainNavLink = mainNavLi.find('a');
		//navLevelTwo = mainNavLi.find('ul')
	mainNavLink.on('click', function(){
		var $this = $(this),
			_thisParent = $this.parent(),
			_thisUl = _thisParent.find('ul, .sub-g-menu');
		if(!$this.hasClass('active')){
			$this.addClass('active');
			_thisParent.siblings().find('ul, .sub-g-menu').slideUp();
			_thisParent.siblings().children('a').removeClass('active');
			_thisUl.slideDown();
		}else {
			$this.removeClass('active');
			_thisUl.slideUp();
	...
	var btnOpenMenu = $('.top-menu__mini').find('i');
	btnOpenMenu.on('click', function(){
		var $this = $(this);
		if(!$this.hasClass('active')){
			$this.addClass('active');
			$('div.top-menu > ul').slideDown();
		} else{
			$this.removeClass('active');
			$('div.top-menu > ul').slideUp();
		}
	})
	//end
